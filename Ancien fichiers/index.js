...
const typing_errors_background = document.getElementById(
  "typing_errors_background"
);
let errors_count = 0;
...
const reset_color = () => {
  document.body.addEventListener("keyup", () => {
    ...
    typing_errors_background.style.background = "#0d6efd";
  });
...
class Afficheur {
  ...
  constructor() {
    ...
    this.step_control = 11;

    this.count_number_of_words = this.count_number_of_words.bind(this);
  }
  nextStep() {
    this.step_control++; // Incrémente la valeur de step_control
  }
  ...
  step_choose() {
    switch (this.step_control) {
      case 1:
        this.execute_saisie("le la", 20, 1);
        break;
      case 2:
  ...
  count_number_of_words(nombre_tours) {
    ...
    /*
        let number_of_words;
         let number_minutes=this.timer.seconds/60;
        
         for(const element of this.array2)
           {
             if(element===' ')
               {
                 count_space++;
           ...
           number_of_words=(count_space+1)*nombre_tours;

          WPM=number_of_words/number_minutes;*/
  }
  ...
  remove_key_background() {
    this.keyboard.set_key_background_for_remove(
      this.array2[this.current_char - 1],
      "#0d6efd"); //Vérification de la lettre actuelle à saisir afin d'enlever sa couleur sur le clavier
  }

  execute_saisie(
    chaine,
    nombre_affichage,
    nombre_tours_d_affichage //Méthode qui va gérer (La vérification du caractère, la saisie et le remplissage du deuxième paragraphe <<Celui qui montre les saisies de l'utilisateur>>)
  ) {
    ...
    document.body.addEventListener("keypress", (event) => {
      //Gestionnaire d'évènement(Keypress) appliqué au document afin de savoir sur quelle touche l'utilisateur a cliqué

      timer_count_verifs++;

      if (timer_count_verifs === 1) {
        //Cette condition a été mise afin de s'assurer que le chronomètre sera lancé uniquement après la première touche de clavier
      ...
      if (event.key == this.array2[this.current_char]) {
        //On vérifie si le caractère saisie par l'utilisateur correspond au caractère actuellement attendu

        this.keyboard.verifLetter_toKeyboard_for_remove(
          this.array2[this.current_char]
        ); //Vérification de la lettre actuelle à saisir afin la mettre en couleur sur le clavier
        this.keyboard.verifLetter_toKeyboard(
          this.array2[this.current_char + 1],
          "#0d6efd"
        ); //Vérification de la lettre actuelle à saisir afin d'enlever la couleur sur le clavier
        ...
        paragraph2.innerHTML = this.array2.substring(0, this.current_char + 1); // Affiche les caractères déjà correctement saisis dans le deuxième paragraphe
        //La méthode substring permet de récupérer une sous chaîne d'une chaîne de caractère et renvoie cette chaîne
        this.current_char++; //Incrémente l'attribut qui représente le caractère actuel afin de passer au caractère suivant
        ...
        this.calcul_pourcent(nombre_tours_d_affichage);
        updateProgressBar(this.pourcent_value);
        ...
        if (
          this.current_char === this.array2.length &&
          this.tour_actuel < this.nombre_de_tours
        ) {
          ...
          document.body.style.backgroundColor = "green"; // Tous les caractères ont été correctement saisis
          this.step_control++;
          this.current_char = 0;
          this.tour_actuel = 1;
          this.nombre_caractere_actuel = 0;
      ...
      } //Si la touche que l'utilisateur touche ne correspond pas à la touche attendue
      else {
        audio.play();
    ...
    reset_color(); //Fonction qui permet de renitialiser la couleur de fond de la page
  }
